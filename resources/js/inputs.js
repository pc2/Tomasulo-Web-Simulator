...
function findRAWDependency(wload, curIndx) {
    var rawDep = [];

    for (let indx = curIndx+1; indx < wload.length; ++indx) {
        if (
            (wload[curIndx].Dest == wload[indx].Src1) ||
            (wload[curIndx].Dest == wload[indx].Src2)
        ) {
            rawDep.push(wload[indx].Id);
        }
    }
...
function findOutputDependency(wload, curIndx) {
    var wawDep = [];

    for (indx = curIndx+1; indx < wload.length; ++indx) {
        if (wload[curIndx].Dest == wload[indx].Dest) {
            wawDep.push(wload[indx].Id);
        }
    }
    return wawDep;
    ...
    function findAntiDependency(wload, curIndx) {
        var warDep = [];
    
        for (indx = curIndx+1; indx < wload.length; ++indx) {
        if ((wload[curIndx].Src1 == wload[indx].Dest) ||
            (wload[curIndx].Src2 == wload[indx].Dest)) {
                warDep.push(wload[indx].Id);
        }
    }
    ...
    function calculateDependency(wload) {
        ...
        wload.forEach(function (element, indx) {
            let dependencyforId = {
                "posAt": indx,
                "insId": element.Id,
                "color":element.Color,
                "raw": findRAWDependency(wload, indx),
                "war": findAntiDependency(wload, indx),
                "waw": findOutputDependency(wload, indx)
            }
